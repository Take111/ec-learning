...
//   実体はこのファイル1つで、Expo Module 側は ios/OrderActivityAttributes.swift のシンボリックリンクで
//   同じ実体を参照する(ファイルを複製して「同期を規律で守る」方式は採らない — 仕組みで安全 > 規律で安全)
struct OrderActivityAttributes: ActivityAttributes {
    struct ContentState: Codable, Hashable {
        ...
        var status: String
        // 進捗トラック上の位置(0 始まり)。段数(stageCount)と合わせて JS 側の時間割から送る —
        // ステージの順序をここで手書きすると時間割と食い違ったときに気づけない
        var stageIndex: Int
    ...
    let totalJpy: Int
    let itemCount: Int
    // 注文確定の時刻。アプリ再起動後に JS 側が進行を引き継ぐための唯一の基準
    let startedAt: Date
    let stageCount: Int
}
