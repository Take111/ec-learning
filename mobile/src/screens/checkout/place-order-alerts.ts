...
export function showPlaceOrderError(
  ...
  if (!(error instanceof ApiError)) {
    Alert.alert("注文に失敗しました", "時間をおいて再度お試しください。");
    return;
  }
  switch (error.body.error) {
    ...
    case "insufficient_stock": {
      ...
      Alert.alert(
        "在庫が不足しています",
        `「${name}」の在庫が注文数量に足りませんでした。\nカートで数量を調整してください。`,
        [{ text: "カートに戻る", onPress: ctx.onBackToCart }],
      );
      return;
    }
    case "network_error":
    ...
    case "invalid_address":
      Alert.alert("配送先を確認できませんでした", "配送先住所が見つかりません。");
      return;
    default:
      break; // 未知コードは switch 後の汎用アラートへ
  }
  Alert.alert("注文に失敗しました", `エラーコード: ${error.body.error}`);
}
